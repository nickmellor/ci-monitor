...
sub device_id_to_string
{
	my $device_type = $_[0];
        ...
        if ($device_type eq $cleware::CUSBaccess::POWER_DEVICE)
	{
                return "Power";
	}
...
# normally the cleware library searches in /dev/bus/usb for files
# $n_devices = cleware::CUSBaccess::OpenCleware.OpenCleware()

# called "hiddev0...15"
# if they are not in that location, you can give a parameter defining
# the string to put in front of the value 0...15
# the following line makes it search for files called /dev/usb/hiddev0...15
# this seems to be the default for the 3.7 kernel on Debian systems
$n_devices = cleware::CUSBaccess::OpenCleware($c, "/dev/usb/hiddev");
